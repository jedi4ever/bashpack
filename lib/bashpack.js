...
function BashPack(config) {
  ...
  this.build = function bashPackBuild(projectDir, startScript ,options,callback) {
    ...
    if (!options.configFile) {
      ...
      configFile = path.join(projectDir,'bashpack.json');
    } else {
    ...
    if (shell.test('-f',outputFile)) {
      if (!settings.force) {
        ...
        logger.info('outputFile "'+outputFile+'" already exists. But we can we remove it as --force has been specified');
        shell.rm(outputFile);
      }
    } else {
    ...
    logger.debug('excludes %s',_settings.exclude);
