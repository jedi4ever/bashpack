...
if test "$APPEND" = y; then
    ...
    OLDENV=`sh "$archname" --bashpack-dumpconf`
    if test $? -ne 0; then
...
exec 3<> "$tmpfile"
(cd "$archdir" && ( tar $TAR_ARGS - $EXCLUDE . | eval "$GZIP_CMD" >&3 ) ) || { echo Aborting: Archive directory not found or temporary file: "$tmpfile" could not be created.; exec 3>&-; rm -f "$tmpfile"; exit 1; }
